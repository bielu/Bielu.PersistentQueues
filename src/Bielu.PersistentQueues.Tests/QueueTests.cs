...
public class QueueTests : TestBase
{
    ...
    public async Task receive_at_a_later_time()
    {
        await QueueScenario(async (queue, token) =>
        {
            ...
            await DeterministicDelay(2000, token).ConfigureAwait(false);
    ...
    public async Task sending_to_bad_endpoint_no_retries_integration_test()
    {
        await QueueScenario(config =>
            ...
            async (queue, token) =>
            {
                ...
                queue.Send(message);
                await DeterministicDelay(5000, token).ConfigureAwait(false); //connect timeout cancellation, but windows is slow
    ...
    public async Task send_batch_of_messages()
    {
        await QueueScenario(async (queue, token) =>
        {
            ...
            await DeterministicDelay(TimeSpan.FromSeconds(1), token).ConfigureAwait(false);
